...
def STL(my_model, exp, place, id, benchmark, mistake):
   ...
   # load IMAC instruction
   IMAC_instruction_path = f"runs/{exp}/{place}_{id}/IMAC_instruction.json"
   if os.path.isfile(IMAC_instruction_path):
      with open(IMAC_instruction_path, 'r') as f:
         d = json.load(f)
         mistake = d['mistake']
         if mistake != 'None.':
            instruction = d['corrected_instruction']
            print('[INFO] IMAC instruction is loaded.')
         else:
            print('[INFO] No mistake in IMAC instruction.')
   else:
      print('[ERROR]')
